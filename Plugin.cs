...
namespace Arsonist
{
    ...
    {
        public override string ModuleName => "The Arsonist Acheivement";
        ...
        public Dictionary<CCSPlayerController, PlayerFireKillCount> _playerKillCount { get; set; } = new();
        ...
        public void CountKill(CCSPlayerController client)
        {
            ...
            if (Config.MaxIncGrenadeKillCount <= 0)
                return;
            ...
            if (_playerKillCount[client!].KillCount >= Config.MaxIncGrenadeKillCount)
            {
                var steamid = client.AuthorizedSteamID?.SteamId64;
        ...
        public async Task TaskComplete(CCSPlayerController client, ulong steamid)
        {
            ...
            if (response != null)
            {
                ...
                Server.NextFrame(() =>
                {
                    client.PrintToChat($" {ChatColors.Green}[Acheivement]{ChatColors.Default} You acheive 'The Arsonist' (Kill enemies with Molotov or IncGreande for {Config.MaxIncGrenadeKillCount} times)");
